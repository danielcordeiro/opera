...
export default function TiposServicoPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    const response = await fetchWithAuth("/api/tipos-servico");
  ...
  const onSubmit = async (values: z.infer<typeof schema>) => {
    await fetchWithAuth("/api/tipos-servico", {
      method: "POST",
  ...
  return (
    <PageShell>
      <div className="grid gap-6">
        <SectionCard title="Novo Tipo de Serviço">
        ...
        <SectionCard title="Tipos de Serviço cadastrados">
          {loading ? (
            <p>Carregando...</p>
          ) : (
            <DataTable
